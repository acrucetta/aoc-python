...
class Grid:
    ...
    def _move_objects(self, obj: Tuple[int, int], direction: Tuple[int, int]):
        """
        Move object and anything ahead of it.
        For the next cell, move it to the one ahead
        of it, until we hit a wall.

        If the object has only walls ahead of it, remain
        it in place.

        If it can move ahead, move the object ahead and
        anything ahead of it.
        """
        dr, dc = direction.to_tuple()  # type: ignore
        r, c = obj
        nr, nc = r + dr, c + dc
